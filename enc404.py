...
def is_valid_encoding(content):
    """Check if the file can be encoded with UTF-8."""
    try:
...
def obfuscate_python(content):
    ...
    obfuscated_code = f"""# Obfuscated by Mr.X
import base64
...
def obfuscate_file(file_path):
    ...
    try:
        ...
        output_file = f"{os.path.splitext(file_path)[0]}_X-enc.{ext}"
        ...
        print(f"\033[91m{' '*1}╔════════════════════════════════════════╗")
        print(f"{'   '*1}Status: Failed By CY404")
        print(f"{'   '*1}Original File: {file_path}")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 X-enc.py <file>")
        sys.exit(1)
